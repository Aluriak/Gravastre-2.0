...
void RenduGraphique::INI_Valeurs() {
    ...
    NAV_InverserZoom = str2num((*val)[2][2]);
    NAV_PasTab = str2num((*val)[2][3]);
    NAV_PrecisionClic = str2float((*val)[2][4]);
    NAV_InverserDefilement = str2num((*val)[2][5]);
    // Valeurs de l'univers
    interG = str2float((*val)[3][0]);
    interBV = str2num((*val)[3][1]);
    // Valeurs de sortie
    musique = (*val)[4][0];
}
